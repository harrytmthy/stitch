...
class ContributionCodeGenerator(private val codeGenerator: CodeGenerator) {
    ...
    fun generate(moduleName: String, registry: Registry) {
        ...
        val packageName = "com.harrytmthy.stitch.generated.${moduleName.toPackageSuffix()}"
        val fileName = "Generated${moduleName.toPascalModuleName()}Contribution"
    ...
    /**
     * Produces a stable string with "s" + "key" format (e.g. "s1a2b3c4d5e6f"), which is used
     * as the package suffix of a generated contribution.
     */
    private fun String.toPackageSuffix(): String {
        val digest = MessageDigest.getInstance("SHA-256").digest(toByteArray())
        val key = digest.take(6).joinToString("") { "%02x".format(it.toInt() and 0xFF) }
